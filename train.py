...
def save_model(cfg, epoch, model, stats):
    ...
    os.makedirs('model_states-balanced', exist_ok=True) #don't overwrite 

    # get model parameters and add to stats...
    stats['model'] = model.state_dict()

    # ...and save
    torch.save(stats, open(f'model_states-balanced/{epoch}.pt', 'wb'))
    
    # also save config file if not present
    cfpath = 'model_states-balanced/config.yaml'
...
def main():
    ...
    while current_epoch < numEpochs:
        ...
        loss_train, oa_train = train(cfg, dl_train, model, optim, classes_for_weight_train)
        loss_val, oa_val = validate(cfg, dl_val, model, classes_for_weighting_val)
